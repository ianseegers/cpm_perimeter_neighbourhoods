...
initialize()
if(Object.keys(C1.getStat(CPM.CellNeighborList)).length == 90 & Object.keys(C2.getStat(CPM.CellNeighborList)).length == 90 & Object.keys(C3.getStat(CPM.CellNeighborList)).length == 90){
	while(calcconnectblue(C1) > 0 || calcconnectblue(C2) > 0 || calcconnectblue(C3) > 0){
		step()
	}
} else {
	console.log("There was an error, so I did not run. Pick a different seed")
}